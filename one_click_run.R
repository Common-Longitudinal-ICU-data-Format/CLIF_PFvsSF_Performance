...
dirs_to_create <- c("markdown_output", "data", "output")
...
# Check which R scripts exist and source them (excluding this script)
r_scripts <- list.files(pattern = "\\.R$", ignore.case = TRUE)
r_scripts <- r_scripts[r_scripts != "one_click_run.R"]
...
cat("\n[4/4] Rendering R Markdown files...\n")
...
cat("2. Check the 'output/' directory for results\n")
